    ...
    <div className="p-4">
      <div className="p-4 border-2 border-gray-200 border-dashed rounded-lg dark:border-gray-700">
        ...
        <div className="flex mt-4 md:mt-6 xl:gap-x-6">
          {
            (songList || []).map(({ songId, tempo, name, tonality, singer, songTonality, songName, songTempo }) => (
              <div key={songId} className="w-80 max-w-sm bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">
                ...
                <div className="flex flex-col items-center pb-10">
                  <h5 className="mb-2 text-2xl font-bold tracking-tight text-gray-900 dark:text-white">{ songName || name }</h5>
                  <span className="text-lg text-gray-500 dark:text-gray-400">{ singer }</span>
                  <span className="text-md text-gray-500 dark:text-gray-400">Tonalidad: { songTonality || tonality }</span>
                  <span className="text-md text-gray-500 dark:text-gray-400">Tempo: { songTempo || tempo } BPM</span>
                </div>
              </div>
            ))
          }
