      ...
      <div className="mt-10 sm:mx-auto sm:w-full sm:max-w-sm">
        <form className="space-y-6" action="#" method="POST">
          ...
          <select id="countries"
            ...
            <option selected value=''>¿Qué tipo de usuario eres?</option>
          ...
          <button type="button"
                  disabled={isDisabled}
                  onClick={async () => {
                    const { status, message } = await login(email, password, userType);
                    dispatchData({ type: status ? 'success' : 'danger', text: message });
                  }}
